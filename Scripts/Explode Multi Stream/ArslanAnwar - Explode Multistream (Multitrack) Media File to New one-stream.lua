...
for selectedMediaItemIndex = 0, selectedMediaItemsCount - 1 do
    ...
    table.insert(mediasToInspect, {mediaItem = mediaItem, file = file, streams = {}})
end
...
for input, stream, description in string.gmatch(fileInformation, "Stream #(%d+):(%d+)(.-)\n") do
    local isVideo = string.match(description, "Video:")
    table.insert(mediasToInspect[tonumber(input) + 1].streams, {isVideo = isVideo})
end

-- Filter only multistream media
...
for _, trackToCreate in ipairs(tracksToCreateList) do
    ...
    for createdTrackIndex = 1, trackToCreate.numberOfChildrenToCreate do
        local createdTrack = reaper.GetTrack(0, trackToCreate.trackNumber + createdTrackIndex - 1)
        reaper.GetSetMediaTrackInfo_String(createdTrack, "P_NAME",
                                           parentTrackName .. " [stream " ..
                                           tostring(createdTrackIndex) .. "]", true)
    end
    local parentTrackDepth = reaper.GetMediaTrackInfo_Value(parentTrack, "I_FOLDERDEPTH")
    reaper.SetMediaTrackInfo_Value(parentTrack, "I_FOLDERDEPTH", 1)
    local lastChildTrack = reaper.GetTrack(0, trackToCreate.trackNumber + trackToCreate.numberOfChildrenToCreate - 1)
    reaper.SetMediaTrackInfo_Value(lastChildTrack, "I_FOLDERDEPTH", parentTrackDepth - 1)
end

-- Insert new items from extracted streams
for _, media in ipairs(mediasToConvert) do
    local track = reaper.GetMediaItemTrack(media.mediaItem)
    local trackNumber = reaper.GetMediaTrackInfo_Value(track, "IP_TRACKNUMBER")
